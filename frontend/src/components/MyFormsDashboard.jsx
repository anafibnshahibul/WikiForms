...
function StatCard({ label, value, color = '#3366cc' }) {
  return (
    <div style={{ background: 'var(--bg)', border: '1px solid var(--border-light)', borderRadius: 8, padding: '14px 18px', textAlign: 'center', flex: 1, minWidth: 100 }}>
...
function MyFormsDashboard({ T, wikiUser, onLogin, onEditForm, onSelectType, onNavigate }) {
  ...
  return (
    <div className="wikiform-container" style={{ marginTop: 24, paddingBottom: 60 }}>
      {/* Header */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border-light)', borderTop: `4px solid ${accent}`, borderRadius: 10, padding: '24px 28px', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ margin: '0 0 4px', fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>📋 My Forms</h1>
      ...
      {showResetNotice && (
        <div style={{ background: '#fffaeb', border: '1px solid #f59e0b', borderLeft: '4px solid #f59e0b', borderRadius: 8, padding: '14px 18px', marginBottom: 16, fontSize: 14, color: '#92400e', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value)}
          style={{ marginLeft: 'auto', padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 12, background: 'var(--bg)', color: 'var(--text-primary)', cursor: 'pointer' }}>
          <option value="updated">Sort: Recent</option>
          <option value="responses">Sort: Most Responses</option>
          <option value="title">Sort: Title</option>
        </select>
        <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
          style={{ padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 12, background: 'var(--bg)', color: 'var(--text-primary)', width: 160 }} />
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map(form => {
            ...
            return (
              <div key={form.slug} style={{ background: 'var(--surface)', border: '1px solid var(--border-light)', borderLeft: `3px solid ${typeColor}`, borderRadius: 10, padding: '16px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: form.content_type === 'quiz' ? '#e6faf5' : '#eaf3ff', color: typeColor, textTransform: 'uppercase' }}>
                        {form.content_type}
                      </span>
                      {!isOwner && (
                        <span style={{ fontSize: 11, fontWeight: 600, color: '#f59e0b', background: '#fffaeb', padding: '2px 8px', borderRadius: 10 }}>Collaborator</span>
                      )}
                    </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                  <a href={`/view/${form.slug}`} target="_blank" rel="noreferrer"
                    style={{ padding: '6px 14px', fontSize: 12, fontWeight: 600, border: '1px solid var(--border)', borderRadius: 6, textDecoration: 'none', color: 'var(--text-primary)', background: 'var(--bg)' }}>
                    👁 View
                  </a>
                  <button onClick={() => onEditForm(form.slug)}
                    style={{ padding: '6px 14px', fontSize: 12, fontWeight: 600, border: `1px solid ${typeColor}`, borderRadius: 6, cursor: 'pointer', color: typeColor, background: 'var(--bg)', fontFamily: 'inherit' }}>
                    ✏️ Edit
                  </button>
                  <button
                    onClick={() => navigator.clipboard.writeText(`${window.location.origin}/view/${form.slug}`)}
                    style={{ padding: '6px 14px', fontSize: 12, fontWeight: 600, border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer', color: 'var(--text-secondary)', background: 'var(--bg)', fontFamily: 'inherit' }}>
                    🔗 Copy Link
                  </button>
                </div>
              </div>
